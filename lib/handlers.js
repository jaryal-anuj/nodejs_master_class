            ...
            if(!err && userData){
                ...
                if(hashedPassword == userData.hashedPassword){
                    ...
                    _data.create('tokens', tokenId, tokenObject, function(err){
                        ...
                        _performance.measure('Token data creating','create data for token','beginning storing token');
                        ...
                        const obs = new PerformanceObserver((items) => {
                            ...
                            items.getEntries().forEach((item) => {
                                debug('\x1b[32m%s\x1b[0m',item.name+' '+item.duration);
                            })
                        })
                        ...
                        if(userChecks.length < config.maxChecks){
                            ...
                            dns.resolve(hostName, function(err, records){
                                if(!err && records){
                                    ...
                                    callback(400, {'Error':'The hostname of the URL entered didnot resolve to any DNS entries'});
                                }
                            });
